...
export default function DashboardPage() {
  ...
  if (screen === "default") {
      ...
      <div className="p-10 space-y-8">
        <div>
          <h1 className="text-3xl font-bold">
            JBC Token Distribution Dashboard
          </h1>
          <p className="mt-3 text-sm text-slate-600 dark:text-slate-100 max-w-2xl">
            ...
            token amounts to distribute in batch. The admin panel will process
            the upload and prepare the distribution.
          </p>
        </div>
        ...
        <section className="rounded-2xl border border-slate-200 p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-100">
            Choose a file with wallet addresses and distribution values.
          ...
          <div className="mt-6 flex flex-col gap-4">
            <div className="flex gap-4 justify-center items-center">
              ...
              <div className="border p-3 rounded-lg">
                <div className="">Estimated Gas Unit</div>
                <div>{gasInfo.units || "-"}</div>
              </div>
              <div className="border p-3 rounded-lg">
                <div className="">Estimated BNB Cost</div>
                <div>{gasInfo.bnb || "-"}</div>
              </div>
              <div className="border border-blue-500 p-3 rounded-lg">
                <div className="">Estimated USD Cost</div>
                <div className="text-blue-500">${gasInfo.usd || "-"}</div>
              </div>
              <div className="border border-green-700 p-3 rounded-lg hover:text-white hover:bg-green-700 text-green-700"
              >
                <div className="">Valid Rows</div>
                <div className="">
                  {validation?.totalValid ?? 0}
                </div>
              </div>
              <div
                className="border border-red-700 p-3 rounded-lg hover:bg-red-700 text-red-700 hover:text-white"
                onClick={() => setScreen("Invalid")}
              >
                <div className="">Invalid Wallets</div>
                <div className="">
    ...
    return (
      <div className="p-5">
        <button onClick={() => setScreen("default")}>
            Back
        </button>
        {validation?.totalInvalid && validation.totalInvalid > 0 && (
          <div className="mt-8 overflow-x-auto">
            {validation.totalInvalid > 0 && (
              <div className="mb-6 flex flex-col gap-4 rounded-xl border border-indigo-100 bg-indigo-50/50 p-4 dark:border-indigo-900/30 dark:bg-indigo-950/20">
                <div className="flex items-center justify-between">
            ...
            <span className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs">
                Displaying{" "}
                {Math.min(rowsPerPage, validation?.invalidRows?.length || 0)}
                of {validation?.invalidRows?.length || 0}
              </span>
            </span>
            <table className="min-w-full divide-y divide-slate-200 text-sm">
              <thead>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1">
                          {row.errors.map((error, i) => (
                            <span key={i} className="text-red-600 text-xs">
                              • {error}
                            </span>
                          ))}
                        </div>
            ...
            {batches[currentBatchIndex]?.rows.length > rowsPerPage && (
              <div className="flex justify-center gap-4 mt-4 py-2 border-t">
                ...
                <span className="text-sm text-slate-500">
                  Page {internalPage + 1} of{" "}
                  {Math.ceil(
                    validation?.totalInvalid ?? 0 / rowsPerPage,
                  )}
                </span>
                <button
                  disabled={
